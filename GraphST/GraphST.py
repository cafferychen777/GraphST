...
class GraphST:
    def __init__(
        ...
        self.features_a = torch.FloatTensor(self.adata.obsm["feat_a"].copy()).to(
            self.device
        )
        ...
        if self.deconvolution:
            ...
            if isinstance(self.adata.X, csc_matrix) or isinstance(
                self.adata.X, csr_matrix
            ):
                self.feat_sp = adata.X.toarray()[:,]
            else:
                self.feat_sp = adata.X[:,]
            if isinstance(self.adata_sc.X, csc_matrix) or isinstance(
                self.adata_sc.X, csr_matrix
            ):
    ...
    def train(self):
        if self.datatype in ["Stereo", "Slide"]:
            ...
            self.model = Encoder(self.dim_input, self.dim_output, self.graph_neigh).to(
                self.device
            )
        ...
        with torch.no_grad():
            ...
            if self.deconvolution:
                ...
                if self.datatype in ["Stereo", "Slide"]:
                    self.emb_rec = self.model(self.features, self.features_a, self.adj)[
                        1
                    ]
                    self.emb_rec = (
                        F.normalize(self.emb_rec, p=2, dim=1).detach().cpu().numpy()
                    )
                else:
                    self.emb_rec = (
                        self.model(self.features, self.features_a, self.adj)[1]
                        .detach()
                        .cpu()
                        .numpy()
                    )
    ...
    def train_sc(self):
        ...
        self.optimizer_sc = torch.optim.Adam(
            self.model_sc.parameters(), lr=self.learning_rate_sc
        )
    ...
    def train_map(self):
        ...
        with torch.no_grad():
            ...
            map_matrix = (
                F.softmax(self.map_matrix, dim=1).cpu().numpy()
            )  # dim=1: normalization by cell
